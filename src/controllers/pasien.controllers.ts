...
export const getAllPasien = async (req: Request, res: Response) => {
  ...
  if (userRole === "Admin" || userRole === "Staff Rekam Medis") {
    try {
      ...
      const totalPages = Math.ceil(totalPasien / limit);
...
export const createPasien = async (req: Request, res: Response) => {
  const data: pasienRequest = req.body;
  const norm = await normAuto();
  try {
    const addPasien = await prisma.pasien.create({
      data: {
        noRm: norm,
        ...data,
      },
...
export const rekamMedis = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  if (!id) {
    ...
    try {
      const totalPasien = await prisma.pasien.count();
      const pasienToday = await prisma.pasien.count({
        where: {
          tglDaftar: {
            gte: startDate,
            lte: endDate,
      ...
      const totalRawatJalan = await prisma.rawatJalan.count();
      const rawatJalanToday = await prisma.rawatJalan.count({
        where: {
          tanggalMasuk: {
            gte: startDate,
            lte: endDate,
  ...
  try {
    ...
    const totalRawatJalanPasien = await prisma.rawatJalan.count({
      where: {
        idPasien: idNumber,
      },
    });

    const RawatJalanPasienToday = await prisma.rawatJalan.count({
      where: {
        idPasien: idNumber,
        tanggalMasuk: {
          gte: startDate,
    ...
    return res.status(200).json(
      responseApi(
        ...
        null,
        {
          totalRawatJalanPasien,
          RawatJalanPasienToday,
        },
      ),
    );
  } catch (error: any) {
